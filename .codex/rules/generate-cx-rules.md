---
name: generate-cn-rules
description: Instructions on how to create new rules
invokable: true
---
...
1. Rules should be stored in the `.codex/rules` directory
2. Rule files must have a `.md` extension (e.g., `my-rule.md`)
3. To reference a file in your rule, use the format: `[filename.ext](md:filename.ext)` where the path is relative to the workspace root
4. Rules use Markdown format with special codex-specific extensions
...
[index.js](md:index.js), which loads configuration from
[config.js](md:config.js)
```
